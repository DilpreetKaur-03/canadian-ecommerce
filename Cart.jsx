 ...
 * - Reads cart from localStorage key "cart" (array of {id, title/name, price, image, qty})
 * - Allows quantity +/- and remove
 * - Shows line totals and subtotal
 * - "Proceed to Checkout" navigates to /checkout/info
 *
 * All styles are inline in this file (no external CSS).
 */
...
export default function Cart() {
  ...
  useEffect(() => {
    try {
      ...
      const normalized = (arr || []).map((it) => ({
        ...
        title: it.title ?? it.name ?? it.title ?? "Product",
  ...
  // Mobile friendly: simple inline responsive tweaks using percentage widths
  const mobileStack = { display: "block" };
              ...
              <div style={summaryBox}>
                ...
                <div style={{ marginTop: 12, display: "flex", gap: 8, justifyContent: "space-between" }}>
                  ...
                  <Link to="/products" style={{ textDecoration: "none" }}>
                    <button style={{ background: "#0a3d62", color: "#fff", padding: "8px 12px", borderRadius: 8, cursor: "pointer" }}>Continue shopping</button>
                  </Link>
                </div>
              </div>
            </div>
